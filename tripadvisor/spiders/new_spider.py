...
class TripadvisorSpider(Spider):
    ...
    def parse_restaurant(self, response):
        for href in \
            response.xpath('//div[starts-with(@class,"quote")]/a/@href'
                           ):
        ...
        if not re.findall(r'or\d', url):
            ...
            next_page = url.replace('or' + str(pagenum), 'or'
                                    + str(pagenum_next))
        yield Request(next_page, meta={'dont_redirect': True},
                      callback=self.parse_restaurant)

    def parse_review(self, response):
        item = TripAdvisorItem()
        item['title'] = response.xpath('.//div/a/span/text()'
                ).extract_first()

        item['content'] = \
            response.xpath('//div[@class="entry"]/p/text()'
                           ).extract_first()
        item['rating'] = \
            response.xpath('.//div[@class="rating reviewItemInline"]/span/@class'
                           ).extract_first()
        date_ = \
            response.xpath('.//div[@class="rating reviewItemInline"]/span/text()'
                           ).extract()
        if len(date_) == 1:
            if re.search('(ago)$', date_[0]) != None:
                date = \
                    response.xpath('.//div[@class="rating reviewItemInline"]/span/@title'
                                   ).extract_first()
            else:
                date = date_[0]
        else:
